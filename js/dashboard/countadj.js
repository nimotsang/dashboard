$(document).ready(function () {
    ...
    var editor = new $.fn.dataTable.Editor({
        ...
        fields: [
            ...
            {
                label: '类型: ', name: 'InventSchedule_Type', type: 'readonly',
                data: function (row) {
            ...
            { label: '门店: ', name: 'Store_Code', type: 'readonly' },
            {
        ...
        if (mode === 'main' && action !== 'remove') {
            ...
            pceditor = new $.fn.dataTable.Editor({
                ...
                fields: [
                    ...
                    { label: 'bin_code: ', name: 'bin_code', type: 'hidden' },
                    { label: 'color_code: ', name: 'color_code', type: 'hidden' },
                    { label: 'size_code: ', name: 'size_code', type: 'hidden' },
                    { label: 'seq_id: ', name: 'seq_id', type: 'hidden' },
                    { label: 'reason_id: ', name: 'reason_id', type: 'hidden' },
                    { label: 'SO_other_sku: ', name: 'SO_other_sku', type: 'hidden' },
                    { label: 'current_unit_price: ', name: 'current_unit_price', type: 'hidden' },
                    { label: 'current_unit_cost: ', name: 'current_unit_cost', type: 'hidden' },
                    { label: '序号: ', name: 'Line_Id' },
                    { label: '序号: ', name: 'ProductCode' },
                    { label: '序号: ', name: 'ProductName' },
                    { label: '序号: ', name: 'Color' },
                    { label: '序号: ', name: 'Size' },
                    { label: '序号: ', name: 'OnHandDisk' },
                    { label: '序号: ', name: 'StoreCount' },
                    { label: '序号: ', name: 'Qty_Adj' },
                    { label: '序号: ', name: 'Qty_Var' },
            ...
            pctable = $("#ProductDetailTable").DataTable({
                ...
                { "data": "OnHandDisk" },
                { "data": "StoreCount" },
                { "data": "Qty_Adj", "className": 'editable' },
                { "data": "Qty_Var" }
                ],
    ...
    editor.on('initEdit', function (e, node, data) {
        ...
        param.pSessionId =Number(editor.field('Session_Id').val());
        param.pStoreCodeId = editor.field('Store_Code_Id').val();
        param.ManualAdjustment = 'YES';
        $.ajax({
            "url": sysSettings.domainPath + "RaymSP_GatewayPayment_GetPISessionDetail",
            "type": "POST",
            "async": true,
            "crossDomain": true,
            "dataType": "json",
            ...
            "success": function (data) {
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    data[i].Color = data[i].color_description;
                    data[i].Size = data[i].size_description;
                    data[i].ProductName = data[i].Short_name;
                    data[i].OnHandDisk = data[i].on_hand_disk;
                    data[i].StoreCount = data[i].Count_1;
                    data[i].Qty_Adj = data[i].qty_adjustment;
                    data[i].Qty_Var =  data[i].Count_1 - data[i].on_hand_disk;
                }
    ...
    table = $("#CountADJTable").DataTable({
        ...
        initComplete: function () {
            table.buttons().container({ className: 'default1' }).appendTo('#CountADJTable_wrapper .col-sm-6:eq(0)');
            initialtable();
        },
        buttons: [
    ...
    function initialtable(store) {
        ...
        $.ajax({
            "url": sysSettings.domainPath + "RaymSP_Gatewaypayment_GetPIADJForStore",
            "async": true,
            "crossDomain": true,
            "type": "POST",
            "dataType": "json",
    ...
    function getpcval(val, key) {
        ...
        var nval = {};
            ...
            nval.pShortSku = Number(val.short_sku);
            nval.pQtyScan = val.size_id;
            nval.pQtyAdjustment = val.inventschedule_id;
            nval.pCountNumber = Number(val.Line_Id);
            nval.pBinId = val.product_id;
